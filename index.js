...
contactsList.addEventListener('click', e => {
  ...
  if (editBtn) {
    ...
    if (status === 'disabled-inputs') {
      ...
      // 4. Agregar eventos de validación a los inputs
      contactInputName.addEventListener('input', validateEditInputs);
      contactInputPhone.addEventListener('input', validateEditInputs);
    }
  
    if (status === 'enabled-inputs') {
      // Validar antes de guardar
      const isNameValid = NAME_REGEX.test(contactInputName.value);
      const isPhoneValid = PHONE_REGEX.test(contactInputPhone.value);
      
      if (!isNameValid || !isPhoneValid) {
        // Mostrar errores si los hay
        renderInputValidationStatus(contactInputName, isNameValid);
        renderInputValidationStatus(contactInputPhone, isPhoneValid);
      ...
      // 4. Eliminar eventos de validación
      contactInputName.removeEventListener('input', validateEditInputs);
      contactInputPhone.removeEventListener('input', validateEditInputs);
      // 5. Actualizar el contacto
      ...
      ContactsModule.saveContactsInBrowser();
      // 7. Mostrar en el html
      ContactsModule.renderContacts(contactsList);
    }
  }
  
  // Función para validar inputs durante la edición
  function validateEditInputs(e) {
